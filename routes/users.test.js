...
describe("GET /users/:username", function () {
  ...
  test("works: user get self", async function () {
    const resp = await request(app)
        .get(`/users/u1`)
  ...
  test("unauth: user get other user", async function () {
    const resp = await request(app)
        .get(`/users/u2`)
        .set("authorization", `Bearer ${u1Token}`);
    expect(resp.statusCode).toEqual(401);
  ...
  test("works: admin get self", async function () {
    const resp = await request(app)
        .get(`/users/u2`)
        .set("authorization", `Bearer ${u2Token}`);
    expect(resp.body).toEqual({
  ...
  test("works: admin get other user", async function () {
    const resp = await request(app)
        .get(`/users/u1`)
        .set("authorization", `Bearer ${u2Token}`);
    expect(resp.body).toEqual({
...
describe("PATCH /users/:username", () => {
  ...
  test("works for admin, give admin rights", async function () {
    const resp = await request(app)
        .patch(`/users/u1`)
        .send({
          firstName: "New",
  ...
  test("forbidden: privilege escalation", async function () {
    const resp = await request(app)
        .patch(`/users/u1`)
        .send({
          isAdmin: true,
...
describe("DELETE /users/:username", function () {
  ...
  test("unauth: delete other user", async function () {
    const resp = await request(app)
        .delete(`/users/u2`)
        .set("authorization", `Bearer ${u1Token}`);
    expect(resp.statusCode).toEqual(401);
